...
def mostrar_modulo_celsia_energia_solar():
    ...
    st.subheader("Estructura esperada del dataset")
    ...
    if precio_bolsa_xm.empty:
        ...
        st.caption("Precio promedio diario de bolsa consultado desde la API oficial de XM.")
    ...
    if not recursos_solares_xm.empty:
        ...
        st.caption(
            "Listado de recursos solares en operacion consultado desde XM; sirve como base para la futura agregacion de generacion solar."
        )
        st.dataframe(
    ...
    if generacion_solar_xm.empty:
        ...
        st.caption(
            "Generacion agregada de plantas solares normales en operacion consultada desde XM."
        )
        st.altair_chart(
    ...
    if not demanda_energia_xm.empty:
        st.subheader("Demanda energetica")
        st.dataframe(demanda_energia_xm, width="stretch", hide_index=True)

    if not generacion_hidraulica_xm.empty:
        st.subheader("Generacion hidraulica")
    ...
    if historico_celsia.empty:
        ...
        if dataset_modelo_celsia.empty or not metricas_modelo_celsia:
            ...
            st.subheader("Dividendos integrados al modelo")
...
def main():
    ...
    with st.sidebar:
        ...
        ticker_petroleo = st.text_input(
            "Ticker de petroleo para regresion",
            value=TICKER_PETROLEO_BRENT,
            help="Por defecto se usa Brent.",
        )
        st.divider()
    ...
    datos_regresion_petroleo = pd.DataFrame()
    metricas_regresion_petroleo = {}
    ...
    if usar_yfinance and not posiciones_vigentes.empty:
        ...
        if "ECOPETROL" in acciones_vigentes:
            try:
                fecha_inicio_ecopetrol = fechas_inicio_yfinance.get("ECOPETROL")
                historico_petroleo = descargar_historico_referencia_yfinance(
                    ticker_petroleo,
                    fecha_inicio_ecopetrol,
                )
                (
                    datos_regresion_petroleo,
                    metricas_regresion_petroleo,
                ) = construir_regresion_lineal_ecopetrol_petroleo(
                    historico_yfinance,
                    historico_petroleo,
                )
            except Exception as error:
                errores_yfinance.append(f"Petroleo ({ticker_petroleo}): {error}")
    ...
    with tablas[5]:
        ...
        if not usar_yfinance:
            ...
            st.subheader("Regresion lineal: ECOPETROL vs petroleo")
            st.caption(
                "Se usa Brent por defecto. Esta regresion trabaja con precios de cierre diarios alineados por fecha."
            )
            if datos_regresion_petroleo.empty or not metricas_regresion_petroleo:
                st.info("No hay suficientes datos alineados entre ECOPETROL y petroleo para calcular la regresion.")
            else:
                r1, r2, r3, r4 = st.columns(4)
                r1.metric("Observaciones", fondo._formatear_numero(metricas_regresion_petroleo["observaciones"]))
                r2.metric("Pendiente", fondo._formatear_numero(metricas_regresion_petroleo["pendiente"]))
                r3.metric("R²", fondo._formatear_numero(metricas_regresion_petroleo["r2"]))
                r4.metric(
                    "Correlacion (-1 a 1)",
                    fondo._formatear_numero(metricas_regresion_petroleo["correlacion"]),
                )
                st.caption(
                    interpretar_correlacion(metricas_regresion_petroleo["correlacion"])
                    + ". -1 indica relacion negativa perfecta, 0 ausencia de relacion lineal y 1 relacion positiva perfecta."
                )
                st.altair_chart(
                    grafico_regresion_ecopetrol_petroleo(datos_regresion_petroleo),
                    width="stretch",
                )
                st.caption(
                    "La comparacion temporal se muestra en indice base 100 para que ECOPETROL y Brent puedan verse en la misma escala."
                )
                st.altair_chart(
                    grafico_series_ecopetrol_petroleo(datos_regresion_petroleo),
                    width="stretch",
                )
